    ...
    for (let i = 0; i < trendings.length; i++) {
        ...
        <a href='detalhes.html?id=${trendings[i].id}&media=${trendings[i].media_type}'>
            ...
            <div class="carousel-caption d-md-block">
                <h3>${trendings[i].tittle ?? trendings[i].name}</h3>
...
containerTopRated.addEventListener("mousemove", (e) => {
    const boudingRect = containerTopRated.getBoundingClientRect();
    const mouseX = e.clientX;

    const threshold = 200; //Distância das bordas para ativar o scroll

    if(mouseX < boudingRect.left + threshold) {
        scrollDirectionTopRated = -1; //Scroll para esquerda
...
function autoScrollTopRated() {
    if (scrollDirectionTopRated !== 0) {
        containerTrendingMovies.scrollLeft += scrollDirectionTopRated * 6; //Ajuste a velocidade (5 = rápido)
